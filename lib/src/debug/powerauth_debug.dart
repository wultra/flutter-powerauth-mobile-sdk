...
class PowerAuthDebug {
    ...
    _nativeLogChannel.receiveBroadcastStream().listen((logData) {
      if (logData is Map) {
        ...
        final timestamp = logData['timestamp'] != null
            ? DateTime.fromMillisecondsSinceEpoch(logData['timestamp'] as int)
            : null;
  ...
  static void traceNativeCodeCalls({
    required traceFailure,
    bool traceEachCall = false,
  }) {
    if (isEnabled) {
      print(
        "PowerAuthDebug: traceNativeCodeCalls is set to traceFailure=$traceFailure, traceEachCall=$traceEachCall",
      );
      if (traceFailure || traceEachCall) {
        MethodChannelHelper.callTracer = DebugCallTracer(
          traceEachCall,
          traceFailure,
        );
      } else {
      ...
      print(
        "PowerAuthDebug: traceNativeCodeCalls is effective only if isEnabled (debug builds) is true.",
      );
    }
  }
  ...
  static Future<void> dumpNativeObjects({String? instanceId}) async {
    if (!isEnabled) {
      print(
        "PowerAuthDebug: dumpNativeObjects is effective only if isEnabled (debug builds) is true.",
      );
      return;
    }
    if (instanceId != null) {
      print(
        "List of native objects associated with instance '$instanceId' = [",
      );
    } else {
    ...
    final maxLenId = objectInfo
        .map((item) => item.id.length)
        .reduce((prev, item) => max(prev, item));
    final maxLenTag = printTag
        ? objectInfo
              .map((item) => item.tag?.length ?? 0)
              .reduce((prev, item) => max(prev, item))
        : 0;
    for (final item in objectInfo) {
      final created = DateTime.fromMillisecondsSinceEpoch(
        item.createDate * 1000,
      );
      final used = item.lastUseDate != null
          ? ", lastUsed='${DateTime.fromMillisecondsSinceEpoch(item.lastUseDate! * 1000)}"
          : '';
      final count = item.usageCount != null ? ", used=${item.usageCount}" : "";
      final valid = item.isValid ? '   ' : '!! ';
      final tag = item.tag != null && instanceId == null
          ? " @ ${item.tag!.padEnd(maxLenTag)}"
          : '';
      final policies = item.policies.join(', ');
      final objId = item.id.padEnd(maxLenId);
      print(
        "  $valid$objId $tag = { ${item.className}, $policies, created='$created'$used$count }",
      );
    }
    print(']');
  }
