...
mixin MethodChannelHelper {
  ...
  Future<T> invokeMethod<T>(
    ...
    return await callTracer.traceCall(
          method,
          arguments,
          () => _invokeMethodChannelMethod<T>(method, arguments, false),
        )
        as T;
  }
  ...
  Future<T?> invokeNullableMethod<T>(
    ...
    return await callTracer.traceCall(
      method,
      arguments,
      () => _invokeMethodChannelMethod<T>(method, arguments, true),
    );
  }

  Future<T?> _invokeMethodChannelMethod<T>(
    String method,
    Map<String, dynamic>? arguments,
    bool nullableResult,
  ) async {
...
class DebugCallTracer implements CallTracer {
  ...
  Future<T?> traceCall<T>(
    ...
    final msg =
        "PowerAuth.$method(${arguments?.entries.map((e) => '${e.key}: ${e.value}').join(', ')})";
    if (_traceCall) {
