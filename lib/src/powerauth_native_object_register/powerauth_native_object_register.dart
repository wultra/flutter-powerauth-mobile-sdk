...
class NativeObjectRegister {
  ...
  static Future<ObjectsCount> countObjects(String tag) async {
    ...
    return ObjectsCount(valid: valid, invalid: r.length - valid);
  }
  ...
  static Future<bool> touchObject(
    String objectId,
    NativeObjectType type,
  ) async {
    return await _platform.debugCommand(
          NativeObjectCmd.touch,
          NativeObjectCmdData(objectId: objectId, objectType: type),
        )
        as bool;
  }
  ...
  static Future<List<NativeObjectInfo>> debugDump(String? instanceId) =>
      _platform.debugDump(instanceId);
}
...
class ObjectsCount {
  ...
  ObjectsCount({required this.valid, required this.invalid});
}

/// Type of native object.
enum NativeObjectType { data, secureData, number, password }

/// Data accepted in debugCommand() function.
class NativeObjectCmdData {
  final String?
  objectId; // object id, accepted in 'release', 'use', 'find', 'touch'
  final String? objectTag; // object tag, accepted in 'create', 'releaseAll'
  final NativeObjectType?
  objectType; // object type accepted in 'create', 'release', 'use', 'find', 'touch'
  final List<String>?
  releasePolicy; // use 'manual', 'after_use N', 'keep_alive T', 'expire T', accepted in 'create'
  final int?
  cleanupPeriod; // cleanup period in milliseconds <100, 60000>, accepted in 'setPeriod'
