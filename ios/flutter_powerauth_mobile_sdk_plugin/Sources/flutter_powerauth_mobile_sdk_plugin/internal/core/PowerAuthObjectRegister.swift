...
internal class PowerAuthObjectRegister {
    ...
    private func findAndRemoveObjects(block: (String, PowerAuthManagedObject) -> Bool) {
        ...
        for (key, value) in register where block(key, value) {
    ...
    func debugDumpObjectsWithTag(tag: String?) -> [[String: Any?]] {
        #if DEBUG
            return lock.synchronized {
...
extension PowerAuthObjectRegister {
    ...
    func usePowerAuthSDK(
        id: String, _ result: @escaping FlutterResult, _ block: (PowerAuthSDK, @escaping WrapThrowBlock) throws -> Void
    ) throws {
