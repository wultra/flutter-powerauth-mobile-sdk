...
class PowerauthBiometricsInteractiveTests extends TestSuiteWithActivation {
  ...
  Future<void> testCreateActivationWithSymmetricKey() async {
    ...
    if (Platform.isAndroid) {
      await showPrompt('Authenticate to create activation with biometry');
    }
    ...
    await expect(
      sdk.tokenStore.requestAccessToken('biometric-token', auth),
    ).toSucceed();
    await expect(
      sdk.tokenStore.removeAccessToken('biometric-token'),
    ).toSucceed();
    ...
    // And add it again
    if (Platform.isAndroid) {
      await showPrompt('Authenticate to add biometric factor again');
    }
    if (Platform.isAndroid) {
      // Symmetric-key setup requires an Android prompt.
      await expect(
        sdk.addBiometryFactor(await credentials.validPasswordObject()),
      ).toThrow(PowerAuthErrorCode.wrongParameter);
    }
    await expect(
      sdk.addBiometryFactor(
        await credentials.validPasswordObject(),
        PowerAuthBiometricPrompt(
          promptTitle: 'Add biometric factor',
          promptSubtitle: 'PowerAuth test',
          promptMessage: 'Authenticate to add the biometric factor',
        ),
      ),
    ).toSucceed();
  ...
  Future<void> testBiometricSignature() async {
    ...
    await expect(sdk.hasBiometryFactor()).toBe(true);
    await showPrompt(
      'Please authenticate with biometry to request access token',
    );
    final auth = PowerAuthAuthentication.biometry(
      biometricPrompt: PowerAuthBiometricPrompt(
        promptMessage: 'Please authenticate with biometry',
    ...
    await expect(
      sdk.tokenStore.requestAccessToken(tokenName, auth),
    ).toBeDefined();
    ...
    await expect(
      sdk.tokenStore.requestAccessToken(tokenName, auth),
    ).toThrow(PowerAuthErrorCode.invalidNativeObject);
  }
  ...
  Future<void> testGroupedBiometricAuthentication() async {
    ...
    await sdk.groupedBiometricAuthentication(credentials.biometry(), (
      reusableAuth,
    ) async {
      //
      await showPrompt(
        'Biometric dialog should not be displayed.',
        duration: UserPromptDuration.quick,
      );
      // Calculate signature
      var data = '{}';
      ...
      // Verify signature
      var result = await helper.verifySignature(
        "POST",
        uriId,
        header.value,
        data,
      ...
      await showPrompt(
        'Biometric dialog should not be displayed again.',
        duration: UserPromptDuration.quick,
      );
      // Calculate yet another signature and verify
      data = '{"value":false}';
  ...
  Future<void> testCancelBiometry() async {
    ...
    final auth = PowerAuthAuthentication.biometry(
      biometricPrompt: PowerAuthBiometricPrompt(
        promptMessage: "Please CANCEL this dialog",
        promptTitle: "Please cancel",
        cancelButtonTitle: "super cancel",
      ),
    );
    await expect(
    ...
    ).toThrow(PowerAuthErrorCode.biometryCancel);
  }
  ...
  Future<void> testFailedBiometry() async {
    ...
    final auth = PowerAuthAuthentication.biometry(
      biometricPrompt: PowerAuthBiometricPrompt(
        promptTitle: "Please fail",
        promptMessage: "Please use wrong biometry to fail",
      ),
    );
  ...
  Future<void> iosTestFallbackButton() async {
    ...
    final auth = PowerAuthAuthentication.biometry(
      biometricPrompt: PowerAuthBiometricPrompt(
        promptTitle: "Please fail",
        promptMessage: "Fail and then click the fallback button",
        fallbackButtonTitle: 'Fallback button',
      ),
    );
    await expect(
      sdk.authenticationHeaderForRequestWithBody(
    ...
    ).toThrow(PowerAuthErrorCode.biometryFallback);
  }

  Future<bool> _isFaceID() async {
    return !Platform.isAndroid &&
        (await sdk.getBiometricStatus()).biometryType ==
            PowerAuthBiometryType.face;
  }
}
