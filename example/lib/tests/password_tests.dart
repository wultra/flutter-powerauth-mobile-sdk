...
class PowerAuthPasswordTests {

    List<Object> cleanup = [];
    List<ExpectResult> results = [];
    ...
    Future<void> runTests() async {
      ...
      var tests = [testAddCharacters, testRemoveCharacters, testInsertCharacters, testUnicode, testManualRelease];
      var successCount = 0;
      ...
      for (var test in tests) {
        print("------------------------");
        print("running ${test.toString().replaceFirst("Closure: () => Future<void> from Function ", "")}");
        beforeEach();
        await test();
        await afterEach();
      }
    ...
    Future<void> afterEach() async {
        ...
        var failedResultsCount = results.where((result) => !result.isResultExpected).length;
        if (failedResultsCount > 0) {
            print("Test finished with ${failedResultsCount} failed asserts out of ${results.length}");
        } else {
            print("All ${results.length} asserts passed");
        }
    }
...
extension on Future<ExpectResult> {
